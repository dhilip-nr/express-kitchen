...
//		var panelElm = new bkElement('DIV').setStyle({width : (parseInt(e.getStyle('width')) || e.clientWidth)+'%'}).appendBefore(e);
...
//			s[(ie7s) ? 'height' : 'maxHeight'] = (this.ne.options.maxHeight) ? this.ne.options.maxHeight+'px' : null;
			var s = {width: '100%', border : '1px solid #ccc', borderTop : 0, overflowY : 'auto', overflowX: 'hidden' };
			s[(ie7s) ? 'height' : 'maxHeight'] = (this.ne.options.maxHeight) ? '140px' : null;
			this.editorContain = new bkElement('DIV').setStyle(s).appendBefore(e);
			var editorElm = new bkElement('DIV').setStyle({width : '98%', background: '#fff', margin: '4px auto', minHeight : newY+'px'}).addClass('main').appendTo(this.editorContain);
...
var nicEditorFontSizeSelect = nicEditorSelect.extend({
	sel : { 2 : '10px', 3 : '12px', 4 : '14px', 5 : '18px'},
	init : function() {
		this.setDisplay('Font&nbsp;Size...');
		for(itm in this.sel) {
			this.add(itm,'<font size="'+itm+'px">'+this.sel[itm]+'</font>');
		}		
	}
});
