...
$grid2->select_command = "SELECT price.id, CONCAT(prd.name,' (W: ',prd.width,' In.)') name, price.material, price.labor, price.margin, price.retail, price.active
FROM remode_company_prices price
LEFT JOIN catalog_products prd on prd.id=price.product_id
WHERE prd.item_group='Wall Cabinets' AND prd.active=1 AND price.company_id='$com_id'";

$grid2->set_columns($cols2);         
$wall_cabinets = $grid2->render("remode_company_prices"); 
