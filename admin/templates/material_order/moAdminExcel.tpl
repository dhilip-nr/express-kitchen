...
<table width="100%">
    <tr>
        <td colspan="2" align="right">ORDER ID : </td>
        <td><b>{$order_result.order_id}</b></td>
        <td width="15%"><b>Customer Name</b> : </td>
        <td colspan="{2+$show_price}">{$order_result.customer_name}</td>
	</tr>
    <tr>
        <td colspan="2" align="right">JOB ID : </td>
        <td><b>{$order_result.jobid} {if $order_result.jobid==""} - {/if}</b></td>
        <td><b>Branch Details</b> : </td>
        <td colspan="{2+$show_price}">{$order_result.branch} - {$order_result.branch_code}
    ...
    <tr bgcolor="#F0F0F0">
		<td colspan="3"><b>Ship To Address</b></td>
        <td colspan="{3+$show_price}"><b>Vendor Details</b></td>
	</tr>
        {$ship_to_info = $MOInfoList->GetOrdHeaderData($order_result, $products_result.0, "ship_to")}
        
        {if !empty($dealers_mat_vendor)}
            {$vendor_info = $dealers_mat_vendor}
        {else}
            {$vendor_info = $MOInfoList->GetOrdHeaderData($order_result, $products_result.0, "vendor")}
        {/if}
    <tr>
        <td colspan="2"><b>Company</b></td>
        <td>{$ship_to_info.company}</td>
        <td><b>Company</b></td>
        <td colspan="{2+$show_price}">{$vendor_info.company}</td>
    </tr>
    <tr>
        <td colspan="2"><b>Name</b></td>
        <td>{$ship_to_info.name}</td>
        <td><b>Name</b></td>
        <td colspan="{2+$show_price}">{$vendor_info.name}</td>
    </tr>
    <tr>
        <td colspan="2"><b>Email</b></td>
        <td>{$ship_to_info.email}</td>
        <td><b>Email</b></td>
        <td colspan="{2+$show_price}">{$vendor_info.email}</td>
    </tr>
    <tr>
        <td colspan="2"><b>Phone</b></td>
        <td>{$ship_to_info.phone}</td>
        <td><b>Phone</b></td>
        <td colspan="{2+$show_price}">{$vendor_info.phone}</td>
    </tr>
    <tr>
        <td colspan="2"><b>Address</b></td>
        <td>{$ship_to_info.address}</td>
        <td><b>Address</b></td>
        <td colspan="{2+$show_price}">{$vendor_info.address}</td>
    </tr>
