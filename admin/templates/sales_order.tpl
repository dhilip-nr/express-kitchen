        ...
        {if !empty($products_result)}
			...
			<tr>
				...
				<td>
					...
					{if $value.options!=""}
						<br>
						{assign var="product_options" value=($value.options|json_decode:1)}
						{foreach from=$product_options key=okey item=ovalue}
							{$okey|ucfirst}: {$ovalue} {if array_key_last($product_options) != $okey} | {/if}
						{/foreach}
					{/if}
				</td>
...
{if $order_result.order_status!="quote" && $order_result.order_status!="saved"}
        <tr>
