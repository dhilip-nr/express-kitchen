	...
	if($mail_sent_y || CW_ENV!="production") {
		...
		if(!$mo_admin_view || in_array($_SESSION[APPSESVAR.'_adminuser']['role'], ['dealer', 'branchadmin'])){
			...
			if(strpos($sent_material, $sending_vendor) !== false){
				...
				$order_update_qry = "update remode_orders set sent_material='".$implode_sent_orders."', dealer_mat_status=if(dealer_mat_status = '', '".date('Y-m-d H:i:s')."', dealer_mat_status) where id =".preg_replace('/'.ORD_PREFIX.'/i', '', $_REQUEST['id']);
			} else {
				$implode_sent_orders = ($sent_material==""? $sending_vendor_wtime : $sent_material.",".$sending_vendor_wtime);
				$order_update_qry = "update remode_orders set sent_material='".$implode_sent_orders."', dealer_mat_status=if(dealer_mat_status = '', '".date('Y-m-d H:i:s')."', dealer_mat_status) where id =".preg_replace('/'.ORD_PREFIX.'/i', '', $_REQUEST['id']);
			}

			$sent_count = count(array_filter(explode(",", $sent_material)));
			$db->query("update remode_order_status set mfg".($sent_count+1)."='".$sending_vendor."' where order_id =".preg_replace('/'.ORD_PREFIX.'/i', '', $_REQUEST['id']));
					
		    $db->query("update remode_order_status set DMS='".date('Y-m-d H:i:s')."' where order_id =".preg_replace('/'.ORD_PREFIX.'/i', '', $_REQUEST['id'])." and TRIM(DMS)=''");
		}

		$db->query("INSERT INTO remode_order_status (order_id) VALUES ('".preg_replace('/'.ORD_PREFIX.'/i', '', $_REQUEST['id'])."') ON DUPLICATE KEY UPDATE order_id = ".preg_replace('/'.ORD_PREFIX.'/i', '', $_REQUEST['id'])."");

	    $db->query($order_update_qry);

		echo '<script language="javascript">';
	    echo 'alert("Material Order Placed Successfully");';
		echo 'window.location = "'.ROOT.'material_order.html?id='.$_REQUEST['id'].'";';
		echo '</script>';
	} else {
		echo '<script language="javascript">';
		echo 'alert("Failed to place Material Order. Please try again.");';
		echo '</script>';
	}
