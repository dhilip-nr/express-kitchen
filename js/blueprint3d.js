...
//                console.log('test')
            ...
            Item.prototype.clickReleased = function () {
               ...
               let wallid='';
            //    console.log('backEdge***',selectedItem)
               if(backEdge.hasOwnProperty('wall'))
                 wallid = backEdge.wall.id;
                else
                wallid=selectedItem.wallId
               
               
                console.log('backEdge',backEdge,mes_info,wallid)
                // if(backEdge){
                    let i = 0
                mes_info.forEach(mes => {
                        ...
                        if (mes.id == wallid ) {
                            ...
                            selectedItem.wallId =  wallid
                            let setWallname = JSON.stringify(mes)
                            $('#change-wall select').val(setWallname)
                        ...
                        if(i==mes_info.length && selectedItem){
                                getMeasureInfo().then(mes=>{console.log('test');
                                buildAvailSpaceVar();
...
(function (BP3D) {
    ...
    (function (Items) {
        var FloorItem = (function (_super) {
            ...
            function setwallNameforFlrItem(){
                return new Promise(resolve=>{
                    let boxForWall=[]
                    mes_info.forEach(res=>{
                let incType = 'z';  
                res.interiorStart['z'] = res.interiorStart.y;
                res.interiorEnd['z'] = res.interiorEnd.y;
                if (Math.abs(res.interiorStart.x - res.interiorEnd.x) + 10 > res.total_space && (Math.abs(res.interiorStart.x - res.interiorEnd.x) - 10 < res.total_space))
                    incType = 'x';
                let min;
                let max;
                let boundBox;
                if(incType=='x'){
                    min ={x:res.interiorStart.x,y:0,z:res.interiorStart.z}
                    max ={x:res.interiorEnd.x,y:res.top,z:(min.z+12)}
                    if ((res.interiorEnd.x - res.interiorStart.x) < 0){

                        min ={x:res.interiorEnd.x,y:0,z:res.interiorStart.y-12}
                    max ={x:res.interiorStart.x,y:res.top,z:res.interiorStart.y}
                    }
                }
                else{
                    min ={x:res.interiorStart.x-12,y:0,z:res.interiorStart.y};
                    max ={x:res.interiorStart.x,y:res.top,z:res.interiorEnd.y};
                    if ((res.interiorEnd.y - res.interiorStart.y) < 0) {
                         min ={x:res.interiorStart.x,y:0,z:res.interiorEnd.y};
                        max ={x:min.x+12,y:res.top,z:res.interiorStart.y};
                    }
                }
                boundBox=new THREE.Box3(min,max);
                boundBox['wall_info']=res;
                boxForWall.push(boundBox);
             
                    })
                    selectedItem.wallId='center';
                    selectedItem['wallName']='None';
                    boxForWall.forEach(box=>{
                        let selItem=selectedItem;
                       let boundingBoxforSel = new THREE.Box3(new THREE.Vector3(), new THREE.Vector3())
                       boundingBoxforSel.setFromObject(selItem);
                        boundingBoxforSel.copy(selItem.geometry.boundingBox).applyMatrix4(selItem.matrixWorld)
                        
                       
                        if( boundingBoxforSel.isIntersectionBox(box)){
                            selectedItem.wallId=box.wall_info.id;
                            selectedItem['wallName']=box.wall_info.name;
                        }
                    })
                    resolve()
                    // console.log('boxForWall',selectedItem.wallName)
                })
            }
            FloorItem.prototype.moveToPosition = function (vec3, intersection) {
                setwallNameforFlrItem().then(_=>{
                    ...
                    B3D.model.checkforvalidPos(boundBox, this.material.uuid).then(data => {
                        ...
                        if (data.isInter || !this.isValidPosition(vec3)) {
    ...
    (function (Items) {
        ...
        var WallItem = (function (_super) {
            ...
            WallItem.prototype.redrawWall = function () {
                if (this.addToWall ) {
                    console.log('wallredraw')
                    // if(this.currentWallEdge.wall)
            ...
            WallItem.prototype.placeInRoom = function () {
                ...
                // console.log('closestWallEdge**',closestWallEdge)
                if (!is_default && this.setWallEdge())
                    closestWallEdge = this.setWallEdge()!=null?this.setWallEdge():closestWallEdge;
                // console.log('closestWallEdge*222*',closestWallEdge)
            ...
            WallItem.prototype.moveToPosition = function (vec3, intersection) {
                ...
                if (incType == 'x') {
                    if (selectedwall.interiorEnd.x - selectedwall.interiorStart.x > 0) {
                        ...
                        max = new THREE.Vector3(vec3.x + this.halfSize.x, vec3.y + this.halfSize.y, min.z + 2 * this.halfSize.z);
                    }
                    else {
...
//                console.log('incType', incType)
                ...
                B3D.model.checkforvalidPos(boundBox, this.material.uuid).then(data => {
                    if (data.isInter) {
                        ...
                        this.hideError();
                        let isCorner=false
                        if(selectedItem.metadata.itemName=='BCCLR')
                            isCorner=true
                        this.changeWallEdge(intersection.object.edge,isCorner);
                        // updateItemWallname(intersection.object.edge).then(_ => {
                            this.boundMove(vec3);
                            this.position.copy(vec3);
                            this.redrawWall();
                    ...
                    backEdge=intersection.object.edge;
                })
            ...
            WallItem.prototype.changeWallEdge = function (wallEdge,corner=false) {
                ...
                // console.log(' wallEdge', wallEdge,this.currentWallEdge)
                ...
                this.rotation.y = angle;
                // if(rot)
                //     this.rotation.y= selectedItem.rotation.y
...
(function (BP3D) {
    ...
    (function (Model) {
        var Scene = (function () {
            ...
            Scene.prototype.removeItem = function (item, dontRemove) {
                ...
                var selectedObject
                selectedObject = this.scene.getObjectByName(item.name);
                console.log('selected', selectedObject)
                if (selectedObject) {
                    this.itemRemovedCallbacks.fire(selectedObject);
                    selectedObject.removed();
                ...
                if (!dontRemove) {
                    ...
                    if (selectedObject)
                        del = selectedObject
                    BP3D.Core.Utils.removeValue(this.items, del);
                }
            };
  ...
  //                      console.log('itemAdd', item)
                        ...
                        let meshname = randomAlphaNumeric(5);
//                        console.log('metadata', meshname)
                        item.fixed = fixed || false;
                        item.show_measure = show_measure || false;
    
                        let res = item;
                        // if(item.name)
                        item.name = 'Mesh-' + meshname
//                        console.log(textureInfo)
                        if (Object.keys(textureInfo).length && textureInfo.textureUrl != '') {
                            ...
                            if(metadata.group!='Appliances' && metadata.group!='Openings')
...
(function (BP3D) {
    ...
    (function (Model_1) {
        var Model = (function () {
            ...
            Model.prototype.checkforWallSpace = function (boundbox) {
                return new Promise(resolve => {
                    ...
                    if (storePos.length) {
                        ...
                        for (i = storePos.length - 1; i >= 0; i--) {
                            if (boundbox.isIntersectionBox(storePos[i]) ) 
                                uuidArray.push(storePos[i].item);
                        }
                        resolve(uuidArray)
                    }
                })
            ...
            Model.prototype.exportSerialized = function () {
                ...
                for (var i = 0; i < objects.length; i++) {
                    ...
                    items_arr[i] = {
                        ...
                        uom:object.metadata.uom,
                        labor_cost:object.metadata.labor_cost,
                        material_cost:object.metadata.material_cost
                    };
                    if (object.metadata.hasOwnProperty('door_config'))
                        items_arr[i]['door_config'] = object.metadata.door_config
        ...
        Three.Controller = function (three, model, camera, element, controls, hud) {
            ...
            this.itemIntersection = function (vec2, item) {
                ...
                customIntersections = item.customIntersectionPlanes();
                   ...
                   console.log('scope',  scope.isRotating(),customIntersections)
                ...
                if (customIntersections && customIntersections.length > 0) {
                    ...
                    console.log('customIntersections',customIntersections,plane)
    ...
    (function (Three) {
        Three.HUD = function (three) {
            ...
            function setWalltype() {
                ...
                if (Math.abs(selectedwall.interiorStart.x - selectedwall.interiorEnd.x) + 10 > selectedwall.total_space && (Math.abs(selectedwall.interiorStart.x - selectedwall.interiorEnd.x) - 10 < selectedwall.total_space))
                    incType = 'x'
                return { type: incType, wallinfo: selectedwall }
            }
            function findLeftnearBy(selItem, disType) {
                let incType = setWalltype()
                let items = B3D.model.scene.getItems();
                let boundingBox;
                let storeItem = [];
                let selItemBound;
                ////to get and store the all the items and prepare bounding boxes sep for selItem vs Other
                items.forEach(res => {
                    boundingBox = new THREE.Box3(new THREE.Vector3(), new THREE.Vector3())
                    boundingBox.setFromObject(res);
                    boundingBox.copy(res.geometry.boundingBox).applyMatrix4(res.matrixWorld);
                    if (res.material.uuid != selItem.material.uuid)
                        storeItem.push(boundingBox);
                    else
                        selItemBound = boundingBox;
                })
                let prev_pos = {};
                if (storeItem.length) {
                    let opp_type = 'x'
                    ...
                    storeItem.forEach(res => {
                        // console.log('check*****', res, selItemBound)

                        ////prepare right and left distance based based on wall 
                        if ((incType.type == 'x' || incType.type == 'z') && (incType.wallinfo.interiorEnd[incType.type] - incType.wallinfo.interiorStart[incType.type] > 0)) {
                            //for left side of the item with wall type like 3and2 
                            if (disType == 'left' && selItemBound.min[incType.type] > res.max[incType.type] &&
                                (((selItemBound.min.y <= res.min.y) && (selItemBound.max.y >= res.min.y)) || ((selItemBound.min.y <= res.max.y) && (selItemBound.max.y >= res.max.y)) || (selItemBound.min.y >= res.min.y && res.min.y < 0)) &&
                                (((Math.round(selItemBound.min[opp_type]) <= Math.round(res.min[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.min[opp_type]))) || ((Math.round(selItemBound.min[opp_type]) <= Math.round(res.max[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.max[opp_type])))
                                )
                            ) {

                                if (Object.keys(prev_pos).length) {
                                    if (prev_pos.max[incType.type] < res.max[incType.type])
                                        prev_pos = res
                                }
                                else
                                    prev_pos = res;
                            ...
                            if (disType == 'left' && selItemBound.max[incType.type] < res.min[incType.type] && (((selItemBound.min.y <= res.min.y) && (selItemBound.max.y >= res.min.y)) || ((selItemBound.min.y <= res.max.y) && (selItemBound.max.y >= res.max.y)) || (selItemBound.min.y >= res.min.y && res.min.y < 0)) &&
                                (((Math.round(selItemBound.min[opp_type]) <= Math.round(res.min[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.min[opp_type]))) || ((Math.round(selItemBound.min[opp_type]) <= Math.round(res.max[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.max[opp_type]))))
                            ) {
                                if (Object.keys(prev_pos).length) {
                                    if (prev_pos.max[incType.type] > res.min[incType.type])
                                        prev_pos = res
                                }
                                else
                                    prev_pos = res;
                    ...
                    //condition to generate and send the distance
                    if (Object.keys(prev_pos).length) {

                        if ((incType.type == 'x' || incType.type == 'z') && ((incType.wallinfo.interiorEnd[incType.type] - incType.wallinfo.interiorStart[incType.type]) > 0)) {
                            let params = {
                                dis: selItemBound.min[incType.type] - prev_pos.max[incType.type],
                                type: incType.type,
                                start: prev_pos.max[incType.type],
                                end: selItemBound.min[incType.type]
                            }
                            ...
                            let params = {
                                dis: prev_pos.min[incType.type] - selItemBound.max[incType.type],
                                type: incType.type,
                                start: selItemBound.max[incType.type],
                                end: prev_pos.min[incType.type]
                            }
                            return (params)
                        }
                    }
                    //distance check point for default satrt and end position
                    else {

                        if ((incType.type == 'x' || incType.type == 'z') && (incType.wallinfo.interiorEnd[incType.type] - incType.wallinfo.interiorStart[incType.type] > 0)) {
                            let params = {
                                dis: selItemBound.min[incType.type] - incType.wallinfo.interiorStart[incType.type],
                                type: incType.type,
                                start: incType.wallinfo.interiorStart[incType.type],
                                end: selItemBound.min[incType.type]
                            }
                            ...
                            let params =
                            {
                                dis: incType.wallinfo.interiorStart[incType.type] - selItemBound.max[incType.type],
                                start: selItemBound.max[incType.type],
                                end: incType.wallinfo.interiorStart[incType.type],
                                type: incType.type
                            }
                            return (params);
            ...
            function findRightnearBy(selItem,) {
                let incType = setWalltype()
                let items = B3D.model.scene.getItems();
                let boundingBox;
                let storeItem = [];
                ...
                if (storeItem.length) {
                    ...
                    storeItem.forEach(res => {
                        ...
                        if ((incType.type == 'x' || incType.type == 'z') && (incType.wallinfo.interiorEnd[incType.type] - incType.wallinfo.interiorStart[incType.type] > 0)) {
                            //for left side of the item with wall type like 3and2 
                            if (selItemBound.max[incType.type] < res.min[incType.type] &&
                                (((selItemBound.min.y <= res.min.y) && (selItemBound.max.y >= res.min.y)) || ((selItemBound.min.y <= res.max.y) && (selItemBound.max.y >= res.max.y)) || (selItemBound.min.y >= res.min.y && res.min.y < 0)) &&
                                (((Math.round(selItemBound.min[opp_type]) <= Math.round(res.min[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.min[opp_type]))) || ((Math.round(selItemBound.min[opp_type]) <= Math.round(res.max[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.max[opp_type])))
                                )
                            ) {
                            ...
                            if (selItemBound.min[incType.type] > res.max[incType.type] && (((selItemBound.min.y <= res.min.y) && (selItemBound.max.y >= res.min.y)) || ((selItemBound.min.y <= res.max.y) && (selItemBound.max.y >= res.max.y)) || (selItemBound.min.y >= res.min.y && res.min.y < 0)) &&
                                (((Math.round(selItemBound.min[opp_type]) <= Math.round(res.min[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.min[opp_type]))) || ((Math.round(selItemBound.min[opp_type]) <= Math.round(res.max[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.max[opp_type]))))
                            ) {
                                if (Object.keys(prev_pos).length) {
                                    if (prev_pos.max[incType.type] < res.max[incType.type])
                    ...
                    //distance check point for default satrt and end position
                        ...
                        if ((incType.type == 'x' || incType.type == 'z') && (incType.wallinfo.interiorEnd[incType.type] - incType.wallinfo.interiorStart[incType.type] > 0)) {
                            let params = {
                                dis: incType.wallinfo.interiorEnd[incType.type] - selItemBound.max[incType.type],
                                type: incType.type,
                                start: selItemBound.max[incType.type],
                                end: incType.wallinfo.interiorEnd[incType.type]
                            }
                            return (params);
                        }
                            ...
                            let params =
                            {
                                dis: selItemBound.min[incType.type] - incType.wallinfo.interiorEnd[incType.type],
                                start: incType.wallinfo.interiorEnd[incType.type],
                                end: selItemBound.min[incType.type],
                                type: incType.type
                            }
                            return (params);
                    ...
                    if ((incType.type == 'x' || incType.type == 'z') && (incType.wallinfo.interiorEnd[incType.type] - incType.wallinfo.interiorStart[incType.type] > 0)) {
                        let params = {
                            dis: Math.round(incType.wallinfo.interiorEnd[incType.type]) - Math.round(selItemBound.max[incType.type]),
                            type: incType.type,
                            start: selItemBound.max[incType.type],
                            end: incType.wallinfo.interiorEnd[incType.type]
                        }
                        ...
                        let params =
                        {
                            dis: selItemBound.min[incType.type] - incType.wallinfo.interiorEnd[incType.type] - 1,
                            start: incType.wallinfo.interiorEnd[incType.type],
                            end: selItemBound.min[incType.type],
                            type: incType.type
                        }
            ...
            function findTopBotnearBy(selItem, sType = '') {
                let incType = setWalltype()
                let items = B3D.model.scene.getItems();
                let boundingBox;
                let storeItem = [];
                ...
                if (storeItem.length) {
                    ...
                    storeItem.forEach(res => {
                        ...
                        if ((incType.type == 'x' || incType.type == 'z')) {
                            if (sType == 'top' && selItemBound.max.y < res.min.y &&
                                ((Math.round(selItemBound.min[incType.type]) <= Math.round(res.min[incType.type]) &&
                                    Math.round(selItemBound.max[incType.type]) >= Math.round(res.min[incType.type])) || (Math.round(selItemBound.min[incType.type]) <= Math.round(res.max[incType.type]) && Math.round(selItemBound.max[incType.type]) >= Math.round(res.max[incType.type]))) &&
                                (((Math.round(selItemBound.min[opp_type]) <= Math.round(res.min[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.min[opp_type]))) || ((Math.round(selItemBound.min[opp_type]) <= Math.round(res.max[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.max[opp_type]))))
                            ) {
                                if (Object.keys(prev_pos).length) {
                                    if (prev_pos.max.y > res.min.y)
                                        prev_pos = res;
                                }
                                else
                                    prev_pos = res;
                            ...
                            else if (sType == 'bottom' && selItemBound.min.y > res.max.y &&
                                ((Math.round(selItemBound.min[incType.type]) <= Math.round(res.min[incType.type]) &&
                                    Math.round(selItemBound.max[incType.type]) >= Math.round(res.min[incType.type])) || (Math.round(selItemBound.min[incType.type]) <= Math.round(res.max[incType.type]) && Math.round(selItemBound.max[incType.type]) >= Math.round(res.max[incType.type]))) &&
                                (((Math.round(selItemBound.min[opp_type]) <= Math.round(res.min[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.min[opp_type]))) || ((Math.round(selItemBound.min[opp_type]) <= Math.round(res.max[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.max[opp_type]))))
                            ) {
                    ...
                    if (Object.keys(prev_pos).length) {
                        if ((incType.type == 'x' || incType.type == 'z')) {
                            let params = {
                                dis: prev_pos.min.y - selItemBound.max.y,
                                type: incType.type,
                                start: selItemBound.max.y,
                                end: prev_pos.min.y
                            }
                            if (sType == 'bottom') {
                                console.log('prev_pos', prev_pos, selItemBound)
                                params.dis = selItemBound.min.y - prev_pos.max.y
                                params.start = prev_pos.max.y,
                                    params.end = selItemBound.min.y
                            }
                            return (params);
                        }
                    }
                    //distance check point for default satrt and end position
                        ...
                        if (incType.type == 'x' || incType.type == 'z') {
                            let params = {
                                dis: incType.wallinfo.top - selItemBound.max.y,
                                type: incType.type,
                                start: selItemBound.max.y,
                                end: incType.wallinfo.top
                            }
                            if (sType == 'bottom') {
                                params.dis = selItemBound.min.y
                                params.start = selItemBound.min.y,
                                    params.end = 0
                            }
                            return (params);
                        }
            ...
            function findnearByforApp(selItem, findfor) {
                let items = B3D.model.scene.getItems();
                let boundingBox;
                let storeItem = [];
                let selItemBound;
                ...
                if (storeItem.length) {
                    ...
                    storeItem.forEach(res => {
                        if ((findfor == 'back' || findfor == 'left') && selItemBound.min[inctype] > res.max[inctype] &&
                            (((selItemBound.min.y <= res.min.y) && (selItemBound.max.y >= res.min.y)) || ((selItemBound.min.y <= res.max.y) && (selItemBound.max.y >= res.max.y)) || (selItemBound.min.y >= res.min.y && res.min.y < 0)) &&
                            (((Math.round(selItemBound.min[opp_type]) <= Math.round(res.min[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.min[opp_type]))) || ((Math.round(selItemBound.min[opp_type]) <= Math.round(res.max[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.max[opp_type])))
                            )
                        ) {
                            ...
                            if (Object.keys(prev_pos).length) {
                                if (prev_pos.max[inctype] < res.max[inctype])
                                    prev_pos = res
                            }
                            else
                                prev_pos = res;
                        ...
                        else if ((findfor == 'front' || findfor == 'right') && selItemBound.max[inctype] < res.min[inctype] &&
                            (((selItemBound.min.y <= res.min.y) && (selItemBound.max.y >= res.min.y)) || ((selItemBound.min.y <= res.max.y) && (selItemBound.max.y >= res.max.y)) || (selItemBound.min.y >= res.min.y && res.min.y < 0)) &&
                            (((Math.round(selItemBound.min[opp_type]) <= Math.round(res.min[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.min[opp_type]))) || ((Math.round(selItemBound.min[opp_type]) <= Math.round(res.max[opp_type])) && (Math.round(selItemBound.max[opp_type]) >= Math.round(res.max[opp_type])))
                            )
                        ) {
                            ...
                            if (Object.keys(prev_pos).length) {
                                if (prev_pos.max[inctype] > res.min[inctype])
                                    prev_pos = res
                            }
                            else
                                prev_pos = res;
                    ...
                    if (Object.keys(prev_pos).length) {
                        let params = {
                            dis: selItemBound.min[inctype] - prev_pos.max[inctype],
                            type: inctype,
                            start: prev_pos.max[inctype],
                            end: selItemBound.min[inctype]
                        }
                        if (findfor == 'front' || findfor == 'right') {
                            params.dis = prev_pos.min[inctype] - selItemBound.max[inctype];
                            params.start = selItemBound.max[inctype];
                            params.end = prev_pos.min[inctype];
                        }
                        ...
                        let params = {
                            dis: selItemBound.min[inctype] - Math.min(...val[inctype]),
                            type: inctype,
                            start: Math.min(...val[inctype]),
                            end: selItemBound.min[inctype]
                        }
                        if (findfor == 'front' || findfor == 'right') {
                            params.dis = Math.max(...val[inctype]) - selItemBound.max[inctype];
                            params.start = selItemBound.max[inctype];
                            params.end = Math.max(...val[inctype])
                        }
                        return params;
                    }
                }
            ...
            function measureLe(item, type = '') {
                var object = new THREE.Object3D();
                if (type)
                    var nearBy = findnearByforApp(item, 'left');
                else
                    var nearBy = findLeftnearBy(item, 'left');

                var color = "#666";
                var geometry = new THREE.Geometry();
                if (nearBy.type == 'x')
                    geometry.vertices.push(new THREE.Vector3(nearBy.start, item.position.y, item.position.z), new THREE.Vector3(nearBy.end, item.position.y, item.position.z));
                if (nearBy.type == 'z')
                    geometry.vertices.push(new THREE.Vector3(item.position.x, item.position.y, nearBy.start), new THREE.Vector3(item.position.x, item.position.y, nearBy.end));
                ...
                //     },
                //     pos: {
                //         x: (item.position.x)-item.halfSize.x-10,
                //         y: item.position.y,
                //         z: (item.position.z)
                //     }
                // }
                ...
                var measure = Math.round(nearBy.dis / 2.54);
                var measure_txt = measure + "\"";
                if (APP_UOM == "FT")
                    measure_txt = Math.floor(measure / 12) + "'" + (measure % 12) + "\"";
                $('#avail-lt').show()
                $('#avail-lt span').text(measure_txt)
                return object;
            ...
            function measureRi(item, type = '') {
                var object = new THREE.Object3D();
                if (type)
                    var nearBy = findnearByforApp(item, 'right');
                else
                    var nearBy = findRightnearBy(item);
                var color = "#666";
                var geometry = new THREE.Geometry();
                if (nearBy.type == 'x')
                    geometry.vertices.push(new THREE.Vector3(nearBy.start, item.position.y, item.position.z), new THREE.Vector3(nearBy.end, item.position.y, item.position.z));
                ...
                if (APP_UOM == "FT")
                    measure_txt = Math.floor(measure / 12) + "'" + (measure % 12) + "\"";
                $('#avail-rt').show()
                $('#avail-rt span').text(measure_txt)
                return object;

            }
            function measureTo(item) {
                var object = new THREE.Object3D();
                var nearBy = findTopBotnearBy(item, 'top');
                var color = "#666";
                var geometry = new THREE.Geometry();
                geometry.vertices.push(new THREE.Vector3(item.position.x, nearBy.start, item.position.z), new THREE.Vector3(item.position.x, nearBy.end, item.position.z));
                var material = new THREE.LineBasicMaterial({
                    color: color,
                    linewidth: 3
                });
                ...
                if (APP_UOM == "FT")
                    measure_txt = Math.floor(measure / 12) + "'" + (measure % 12) + "\"";
                $('#avail-top').show();
                $('#avail-top span').text(measure_txt)
                // let params = {
                //     dis: {
                //         x: item.position.x,
                //         y: nearBy.dis,
                ...
                //     pos: {
                //         x: item.position.x,
                //         y: item.position.y + item.halfSize + 10,
                //         z: item.position.z
                //     }
                // }
            ...
            function measureBo(item) {
                var object = new THREE.Object3D();
                var nearBy = findTopBotnearBy(item, 'bottom');
                ...
                if (APP_UOM == "FT")
                    measure_txt = Math.floor(measure / 12) + "'" + (measure % 12) + "\"";
                $('#avail-btm').show();
                $('#avail-btm span').text(measure_txt)
                return object;
                // let params = {
                //     dis: {
                //         x: item.position.x,
            ...
            function measureFr(item) {
                var object = new THREE.Object3D();
                var nearBy = findnearByforApp(item, 'front');
                ...
                // console.log('Left Measure Mark')
                var line = new THREE.Line(geometry, material, THREE.LinePieces);
                line.name = "Back Measure Mark Appl";
                object.add(line);
                // let params = {
                //     dis: {
                //         x: nearBy.dis,
                ...
                var measure = Math.ceil(nearBy.dis / 2.54);
                var measure_txt = measure + "\"";
                if (APP_UOM == "FT")
                    measure_txt = Math.floor(measure / 12) + "'" + (measure % 12) + "\"";
                $('#avail-front').show()
                $('#avail-front span').text(measure_txt)
                return object;
            }
            function measureBa(item) {
                var object = new THREE.Object3D();
                var nearBy = findnearByforApp(item, 'back');
                ...
                if (APP_UOM == "FT")
                    measure_txt = Math.floor(measure / 12) + "'" + (measure % 12) + "\"";
                $('#avail-back').show()
                $('#avail-back span').text(measure_txt)
                return object;
            }
            function showMeasure(item) {
                if (["Appliances"].indexOf(item.metadata.group) == -1) {
                    if (item.metadata.itemType == 9 || item.metadata.itemType == 7)
                        activeMeasures = [measureLe(item), measureRi(item), measureTo(item)];
                    else if (item.metadata.itemType == 3 || item.metadata.itemType == 2)
                        activeMeasures = [measureLe(item), measureRi(item), measureTo(item), measureBo(item)];
                    activeMeasures.forEach((obj, idx) => {
                        obj.name = 'Line-idx' + idx
                        line_name.push(obj)
                        if (obj !== null)
                            scene.add(obj)
                    })
                    ...
                    activeMeasures = [measureLe(item, 'appl'), measureBa(item), measureRi(item, 'appl'), measureFr(item)];
                    activeMeasures.forEach((obj, idx) => {
                        obj.name = 'Line-idx' + idx
                        line_name.push(obj)
                        if (obj !== null)
    ...
    (function (Three) {
        Three.Main = function (model, element, canvasElement, opts) {
            ...
            this.setWallName = function (obj) {
                ...
                // console.log('prev_wallObj',prev_wallObj)
