...
var set_dr_flag = { "Wall Cabinets": {}, "Base Cabinets": {}, "Tall Cabinets": {} }
...
function itemSelected(item) {
    ...
    if (item.metadata.group == "Base Cabinets" || item.metadata.group == "Wall Cabinets" || item.metadata.group == "Tall Cabinets")
...
var SideMenu = function (blueprint3d, floorplanControls, modalEffects) {
    ...
    function floorplanUpdate() {
        ...
        window.getMeasureInfo(blueprint3d);
        setCurrentState(scope.states.DEFAULT);
    ...
    function initItems() {
        $("#add-items, #add-standard-items").find(".add-item").mousedown(function (e) {
            ...
            if ($(this).attr("model-status") == 1) {
                if (metadata.group == 'Base Cabinets' || metadata.group == 'Tall Cabinets' || metadata.group == 'Wall Cabinets') {
...
//        console.log('grpname', groupname, selectedItem.metadata.group, configWall)
        if (apply_all) {
            ...
            if (items.length) {
                items.forEach(res => {
                    if (res.metadata.group == groupname) {
        ...
        html2canvas(element, {
            onrendered: function (canvas) {
                ...
                if (imageData) {
                    ...
                    $("#saveFile").attr("download", "image.jpg").attr("href", newData);
                }
            }
        });
    }
        ...
        sum_items.forEach(res => {
            ...
            if (["Openings", "Appliances"].indexOf(grp_name) == -1) {
                let buildData = {
                    ...
                    ypos: res.position.y,
                    zpos: res.position.z,
                    door_mat: res.metadata.door_config.material.mat_name,
                    door_style: res.metadata.door_config.doors.long_desc,
                    door_col: res.metadata.door_config.colors.color_name,
                    drawer_front: res.metadata.door_config.drawer.name,
                    handle_col: res.metadata.door_config.handle_col.short_desc,
                    corbel: res.metadata.door_config.corbels.short_desc,
                    molding: res.metadata.door_config.moldings.short_desc,
                    valance: res.metadata.door_config.valances.short_desc,
                    floating_shelf: res.metadata.door_config.floating_shelf.short_desc,
                    misc_item: res.metadata.door_config.misc_item.short_desc,
                }
            ...
            if (sum_type == 'prev_summary') {
                if (Object.keys(prev_item).length && ((prev_item.sku + prev_item.width + prev_item.height + prev_item.depth != sum_item.sku + sum_item.width + sum_item.height + sum_item.depth) || (prev_item.door_mat != sum_item.door_mat || prev_item.door_style != sum_item.door_style || prev_item.door_col != sum_item.door_col || prev_item.drawer_front != sum_item.drawer_front || prev_item.handle_col != sum_item.handle_col || prev_item.misc_item != sum_item.misc_item || prev_item.corbel != sum_item.corbel || prev_item.molding != sum_item.molding || prev_item.floating_shelf != sum_item.floating_shelf || prev_item.valance != sum_item.valance))) {
                    ...
                    html += '<div class="prev-sum-body"><img src="' + (prev_item.image_url ? prev_item.image_url : "") +
                        ...
                        '<br><label>Style:</label> ' + prev_item.door_style +
                        ' | <label>Color:</label> ' + prev_item.door_col +
                        '<br><div class="pricer"> <label>Qty:</label> ' + (qty - 1) + ' | <label>Price:</label> $' + Number(qty - 1) * Number(prev_item.price) +
                        '<a data-item=' + res + ' onClick=showMore("' + res + '",' + qty + ')>More</a></div></div></div>';
                    qty = 1;
                    // }
                ...
                if (grouped_items[grp][grouped_items[grp].length - 1] == sum_item) {
                    ...
                    html += '<div class="prev-sum-body"><img src="' + (sum_item.image_url ? sum_item.image_url : "") +
                        ...
                        '<br><label>Style:</label> ' + sum_item.door_style +
                        ' | <label>Color:</label> ' + sum_item.door_col +
                        '<br><div class="pricer"> <label>Qty:</label> ' + qty + ' | <label>Price:</label> $' + Number(qty) * Number(sum_item.price) +
                        '<a data-item=' + res + ' onClick=showMore("' + res + '",' + qty + ')>More</a></div></div></div>';
                    qty = 0;
                }
            } else {
                if (Object.keys(prev_item).length && ((prev_item.sku + prev_item.width + prev_item.height + prev_item.depth != sum_item.sku + sum_item.width + sum_item.height + sum_item.depth) || (prev_item.door_mat != sum_item.door_mat || prev_item.door_style != sum_item.door_style || prev_item.door_col != sum_item.door_col || prev_item.drawer_front != sum_item.drawer_front || prev_item.handle_col != sum_item.handle_col || prev_item.misc_item != sum_item.misc_item || prev_item.corbel != sum_item.corbel || prev_item.molding != sum_item.molding || prev_item.floating_shelf != sum_item.floating_shelf || prev_item.valance != sum_item.valance))) {
                    if (Object.keys(prev_item).length) {
                        html += '<div class="sum-body"><img src="' + (prev_item.image_url ? prev_item.image_url : "") +
                            ...
                            '<label>H:</label> ' + prev_item.height + (APP_UOM == 'IN' ? ' In. ' : ' ') +
                            '<label>D:</label> ' + prev_item.depth + (APP_UOM == 'IN' ? ' In. ' : ' ') +
                            '<br><label>Material:</label> ' + prev_item.door_mat +
                            ...
                            html += '</div><a style="margin-left:10px" class="more-rev">More</a><a style="display:none;margin-left:10px;"     class="hide-rev">Hide</a>' +
                            '</div><div class="pricer"><label>Qty:</label> ' + (qty-1) + '<br><label>Price:</label> $' + (Number(qty) * Number(prev_item.price)).toFixed(2) +
                ...
                if (grouped_items[grp][grouped_items[grp].length - 1] == sum_item) {
                    ...
                    html += '</div><a style="margin-left:10px" class="more-rev">More</a><a style="display:none;margin-left:10px;"     class="hide-rev">Hide</a>' +
                        '</div><div class="pricer"><label>Qty:</label> ' + qty + '<br><label>Price:</label> $' + (Number(qty) * Number(sum_item.price)).toFixed(2) +
                        //                  '<br><br><span><a href="#" class="glyphicon glyphicon-pencil"></a> &nbsp; <a href="#" class="glyphicon glyphicon-trash"></a></span>'+
...
function showMore(item, qty) {
    ...
    more_html = '<div class="modal-content"><span class="close">&times;</span><h2>' + res.group + '</h2><h4>General</h4><label>Name:</label> ' + res.name +
        ...
        '<label>D:</label> ' + res.depth + (APP_UOM == 'IN' ? ' In. ' : ' ') +
        '<div style="float:right"><label>Qty:</label>' + qty + '</div><h4>Door Config</h4>' +
...
var mes_info = []
function getMeasureInfo(blueprint3d) {
    var wallItems = blueprint3d.model.floorplan.getWalls();
    //       console.log('wallItems', wallItems)
    if (wallItems.length) {
        mes_info = []
        let i = 1
        // var measureType= $('option:selected').val();
        wallItems.forEach(wall => {
            var occu_space = window.buildAvailSpace(blueprint3d, wall.items, wall.onItems, wall.height);
            //            console.log("wall items: ", wall.items)
            // occu_space.occ_space = window.cmToIn(occu_space.occ_space);   n
            // console.log('wallStart',wall.getStart(), wall.getEnd())
            var x = wall.getEnd().x - wall.getStart().x
            var y = wall.getEnd().y - wall.getStart().y

            var tot_space = wall.backEdge ? wall.backEdge.interiorDistance() : wall.frontEdge.interiorDistance();

            // if(x==0)
            // 	tot_space = (Math.abs(y)-wall.thickness);
            // if(y==0)
            // 	 tot_space =(Math.abs(x)-wall.thickness);

            //  console.log('tot_space',x,y)
            ...
            let buildWall_meas = {
                ...
                center: wall.backEdge ? wall.backEdge.interiorCenter() : wall.fronEdge.interiorCenter(),
                top: wall.height,
                interiorEnd: wall.backEdge.interiorEnd(),
                interiorStart: wall.backEdge.interiorStart()
            }
            // console.log(buildWall_meas)
            mes_info.push(buildWall_meas)
        })
        //    console.log(' mes_info', mes_info);
        ...
        mes_info.forEach((w_space, index) => {
            ...
            //     w_space.y=-40.5;
            // }
            //            console.log("w_space...",w_space.center, "index:", index);
            if (index == 2)
        ...
        if (wallID == '') {
            ...
            blueprint3d.three.setWallName(setWallname);
...
function setWallSizeVal() {
    if ($('#wall-ht option:selected').text() == 'Custom') {
        // console.log("APP_UOM", APP_UOM);
        let in_cm = (APP_UOM == 'IN' ? window.inToCm($("#cust-wall-inp").val()) : window.ftToCm($("#cust-wall-inp").val()));

        $('#wall-ht option:selected').val(in_cm)
    }
}
function buildAvailSpace(blueprint3d, item, onItem, ht) {
    //    console.log("build ava" ,ht,item,onItem,blueprint3d)
    // let = 
    let merge_item = [];
    let active_item = [];
    let occ_space = { wall: { w_cab: 0, w_item_no: 0 }, base: { b_cab: 0, b_item_no: 0 } };
    var sum_items = blueprint3d.model.scene.getItems();
    sum_items.forEach(item => { active_item.push(item.id) })
    item.forEach(res => { merge_item.push(res) });
    onItem.forEach(res => { merge_item.push(res) });
    merge_item.forEach(mer => {
        if (active_item.indexOf(mer.id) != -1) {
            let posY_top = mer.position.y + (mer.sizeY / 2);
            let posY_btm = mer.position.y - (mer.sizeY / 2);
            let ht_top = ht - (30.48 * 3.5);
            let ht_btm = 30.48 * 2.5;
            let val_top = window.inRange(posY_top, ht_top, ht);
            let val_btm = window.inRange(posY_btm, 0, ht_btm);
            // console.log('condition',val_top,val_btm)
            if (val_top) {
                occ_space.wall.w_cab += (mer.sizeX)
                occ_space.wall.w_item_no += 1;
            }
            else if (val_btm) {
                occ_space.base.b_cab += (mer.sizeX)
...
var qty = function(INCH, FT, type) {
	return new Promise(resolve => {
		...
		console.log('valMeas', INCH, FT)
		if (APP_UOM == 'IN' && Number(valMeas.inc_dec)) {
			if (type == '-') {
				value -= valMeas.inc_dec;
				if (Number(valMeas.min) > Number(value)) {
					value = valMeas.min;
					measureAlert('' + value + ' In is the least measurement value for the selected Item')
				}
			} else if (type == '+') {
				value += valMeas.inc_dec;
				if (Number(valMeas.max) < Number(value)) {
					value = valMeas.max;
					measureAlert('' + value + ' In is the most measurement value for the selected Item')
				}
			}
			setTimeout(function () {
...
function validateIncsizePos(type, val) {
    ...
    return new Promise(resolve => {
        ...
        console.log('incType', incType)
        if (incType == 'x') {
            minR = new THREE.Vector3((selectedItem.position.x + 2 - selectedItem.halfSize.x), selectedItem.position.y - selectedItem.halfSize.y, selectedItem.position.z - selectedItem.halfSize.z)
            ...
            boxR = new THREE.Box3(minR, maxR)
            boxArry.push(boxR)
            maxL = new THREE.Vector3((selectedItem.position.x - 2 + selectedItem.halfSize.x), selectedItem.position.y + selectedItem.halfSize.y, selectedItem.position.z + selectedItem.halfSize.z)
        ...
        if (incType == 'z') {
            ...
            minR = new THREE.Vector3((selectedItem.position.x - selectedItem.halfSize.z), selectedItem.position.y - selectedItem.halfSize.y, selectedItem.position.z + 2 - selectedItem.halfSize.x)
            ...
            minL = new THREE.Vector3(Math.round(maxL.x + (type == 'depth' ? val : (2 * selectedItem.halfSize.z))), maxL.y + (type == 'height' ? val : (2 * selectedItem.halfSize.y)), ((selectedItem.position.z - 2) + selectedItem.halfSize.x))
            ...
            if (incType == 'x') {
                if ((selectedwall.interiorEnd.x - selectedwall.interiorStart.x) > 0) {
                    if (!data.isInter && data.boundBox.max[incType] < selectedwall.interiorEnd[incType] && data.boundBox.max[incType] > selectedwall.interiorStart[incType]) {
                        selectedItem.position.set(data.pos.x - 2, data.pos.y, data.pos.z)
                        resolve(true)
                    }
                    else {
                        B3D.model.checkforvalidPos(boxArry[1], selectedItem.material.uuid).then(data1 => {
                            if (!data1.isInter && data1.boundBox.min[incType] < selectedwall.interiorEnd[incType] && data1.boundBox.min[incType] > selectedwall.interiorStart[incType]) {
                                selectedItem.position.set(data1.pos.x + 2, data1.pos.y, data1.pos.z)
                                resolve(true)
                            }
                            else
                                resolve(false)
                ...
                if ((selectedwall.interiorEnd.y - selectedwall.interiorStart.y) > 0) {
                    ...
                    if (data.isInter == false && data.boundBox.max[incType] < selectedwall.interiorEnd[incType] && data.boundBox.max[incType] > selectedwall.interiorStart[incType]) {
                        selectedItem.position.set(data.pos.x, data.pos.y, data.pos.z - 2)
                        resolve(true)
                    }
                    else {
                        B3D.model.checkforvalidPos(boxArry[1], selectedItem.material.uuid).then(data1 => {
                            ...
                            if (data1.isInter == false && data1.boundBox.min[incType] < selectedwall.interiorEnd[incType] && data1.boundBox.min[incType] > selectedwall.interiorStart[incType]) {
                                selectedItem.position.set(data1.pos.x, data1.pos.y, data1.pos.z + 2)
                                resolve(true)
                            }
                            else
                                resolve(false)
